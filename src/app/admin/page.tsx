...
import { getAdminDashboardStats, listAdminOrders } from "@/lib/admin/dashboardData";
...
export default async function AdminPage() {
  await requireAdmin();
  const [stats, orders] = await Promise.all([getAdminDashboardStats(), listAdminOrders()]);
          ...
          <table className="w-full min-w-[780px] text-left text-sm">
            <thead className="text-white/55">
              <tr className="border-b border-white/10">
                ...
                <th className="py-3 pr-4">Quartier</th>
              ...
              {orders.length === 0 ? (
                ...
                (orders as AdminOrderRow[]).map((o) => (
                  <tr key={o.id} className="border-b border-white/6">
                    ...
                    <td className="py-3 pr-4 text-white/70">{o.neighborhood_name}</td>
